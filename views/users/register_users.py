...
class FormularioRegistroEdicion(ctk.CTkFrame):
    def __init__(self, parent, user_controller=None, usuario_a_editar=None, username=None, rol=None, volver_callback=None):
        ...
        self.rol = rol
        # si no se pasa un controlador, crearlo con el contexto del usuario actual
        self.user_controller = user_controller or UserController(current_username=username, current_role=rol)
        ...
        if self.usuario_a_editar:
            ...
            # setea el rol existente (debe coincidir con las opciones)
            try:
                self.role_combobox.set(self.usuario_a_editar.rol)
            except Exception:
                # si por alguna razon falla el set, ignorar y dejar vacío
                pass

            # Si el usuario logueado es aprendiz, solo puede editar su propio usuario y no cambiar rol
            if self.rol == "aprendiz":
                if self.usuario_a_editar.username != self.username:
                    messagebox.showwarning("Permiso denegado", "No puedes editar otros usuarios.")
            ...
            # por defecto ocultar campos de contraseña hasta que marque el checkbox
    ...
    def _save_user(self):
        ...
        # obtener rol: si viene vacío en edición, usar rol existente del usuario a editar
        raw_rol = (self.role_combobox.get() or "")
        rol = raw_rol.lower().strip() if raw_rol else (self.usuario_a_editar.rol if self.usuario_a_editar else "")

        # validación básica: en creación rol es obligatorio; en edición ya hay fallback
        if not username or (not rol and not self.usuario_a_editar):
        ...
        if not self.usuario_a_editar:
            ...
            if hasattr(self, "change_password_checkbox") and self.change_password_checkbox.get():
            ...
            try:
                # la firma del controlador: actualizar(user_id, username, rol, new_password=None)
                self.user_controller.actualizar(
