...
class BuscarUsuarios(ctk.CTkFrame):
    def __init__(self, parent,username,rol, volver_callback):
        ...
        # Frame principal para toda la vista de gestión
        # ==== Tarjeta principal ====
        ...
        self.create_button = ctk.CTkButton(self, fg_color="#06A051", hover_color="#048B45", text="Crear Usuario",height=40, font=("Georgia", 16),
                                        text_color="black",width=200#,command=self.crear_usuario
        )
    ...
    def filtrar_usuarios(self, event=None):
        ...
        for widget in self.card_frame.winfo_children()[2:]:
            widget.destroy()
    ...
    def crear_tarjeta_usuario(self, usuario, row, col):
        ...
        edit_button = ctk.CTkButton(button_frame,height=40,font=("Georgia", 14), fg_color="#06A051", hover_color="#048B45", text_color="black", text="Editar", command=lambda: self.editar_usuario(usuario))
        edit_button.grid(row=0, column=0, padx=(0, 5), sticky="ew")
        
        delete_button = ctk.CTkButton(button_frame,height=40,font=("Georgia", 14), fg_color="#D12B1B", hover_color="#B81D0F", text_color="black", text="Eliminar",
    ...
    #    form_window = UserFormToplevel(self, self.user_controller, on_submit=self.cargar_usuarios)
    #    form_window.grab_set()

    #def editar_usuario(self, usuario):
    #    form_window = UserFormToplevel(self, self.user_controller, usuario_para_editar=usuario, on_submit=self.cargar_usuarios)
    #    form_window.grab_set()
    ...
    def confirmar_eliminacion(self, user_id):
        if messagebox.askyesno("Confirmar eliminación", "¿Estás seguro de que deseas eliminar este usuario?"):
            try:
                # Se llama al método de eliminación del controlador
                self.user_controller.eliminar(user_id)
                # Si la eliminación es exitosa, se muestra el mensaje y se recargan los usuarios
                messagebox.showinfo("Éxito", "Usuario eliminado exitosamente.")
                self.cargar_usuarios()
            except Exception as e:
                # Si ocurre un error, se muestra un mensaje de error detallado
